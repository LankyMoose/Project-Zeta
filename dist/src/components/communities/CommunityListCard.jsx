...
export const CommunityListCard = (props) => {
    const { members, community } = props;
    const nMembers = parseInt(members.toString());
    return (<div className="card community-card" key={community.id}>
      <div className="card-title flex justify-content-between">
        <div className="flex gap align-items-center">
          <h2 className="m-0 line-height-1">
            <a href={`/communities/${community.url_title}`} onclick={(e) => {
            e.preventDefault();
          ...
          {community.private ? (<span className="badge bg-primary-light text-light">Private</span>) : (<></>)}
        </div>
