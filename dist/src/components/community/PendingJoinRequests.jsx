...
const JoinRequestCard = (joinReq) => {
    const { id, communityId } = joinReq;
    const loading = createSignal(false);
    const respondToRequest = async (communityId, joinRequestId, accept) => {
        loading.value = true;
    ...
    return (<div key={id} className="card flex-row align-items-center">
      ...
      <button type="button" className="btn btn-primary" watch={loading} bind:disabled={() => loading.value} onclick={() => respondToRequest(communityId, id, true)}>
        Accept
      </button>
      <button type="button" className="btn btn-danger" watch={loading} bind:disabled={() => loading.value} onclick={() => respondToRequest(communityId, id, false)}>
        Reject
      </button>
    </div>);
};
