...
import { communityDeleteModalOpen, communityHasMembers, pathStore, selectedCommunity, } from "../../state";
...
export const CommunityDeleteConfirmation = () => {
    ...
    return (<Modal visible={communityDeleteModalOpen} toggle={() => (communityDeleteModalOpen.value = false)}>
      <ModalHeader>
      ...
      <ModalBody watch={selectedCommunity} bind:children>
        ...
        <input type="text" className="form-control" watch={confirmText} bind:value={() => confirmText.value} oninput={(e) => (confirmText.value = e.target.value)}/>
      </ModalBody>
      <ModalFooter>
        <Button type="button" watch={loading} bind:disabled={() => loading.value} className="btn btn-secondary hover-animate" onclick={() => (communityDeleteModalOpen.value = false)}>
          Cancel
        </Button>
        <Button type="button" watch={[confirmText, loading]} bind:disabled={() => confirmText.value !== selectedCommunity.value?.title || loading.value} className="btn btn-danger hover-animate" onclick={handleDelete}>
          Delete
