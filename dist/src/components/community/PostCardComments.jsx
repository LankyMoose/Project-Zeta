...
const CommentsList = ({ comments }) => {
    return (<div className="comments-list">
      <p className="text-muted m-0" watch={comments} bind:visible={() => comments.value.length === 0}>
        <small>
          <i>No comments yet.</i>
        </small>
      </p>
...
export const PostCardComments = ({ post }) => {
    const comments = computed(post, () => post.value.comments);
    return (<div className="post-card-comments flex flex-column gap">
...
const NewCommentForm = ({ post }) => {
    ...
    return (<form className="flex align-items-center gap flex-wrap justify-content-end" onsubmit={handleSubmit}>
      <div className="flex align-items-center gap flex-wrap flex-grow">
        ...
        <div className="flex-grow">
          <textarea className="form-control" placeholder="Write a comment..." watch={newComment} bind:value={() => newComment.value} oninput={handleInput}/>
        </div>
      </div>
      <Button watch={[loading, newComment]} bind:disabled={() => loading.value || !commentValidation.isCommentValid(newComment.value)} type="submit" className="btn btn-primary hover-animate">
        Add Comment
