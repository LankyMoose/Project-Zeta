...
const AuthModalProviderList = () => {
    ...
    return (<div className="flex gap flex-column text-center">
      {options.map((option) => (<a href={`/login/${option.title.toLowerCase()}`} className="btn flex gap-sm p-3 auth-provider shadow" onclick={(e) => {
                e.preventDefault();
...
export const AuthModal = () => {
    return (<Modal visible={authModalOpen} toggle={() => (authModalOpen.value = false)}>
      <ModalHeader>
      ...
      <ModalFooter>
        <Button className="btn btn-secondary hover-animate" onclick={() => (authModalOpen.value = false)}>
          Cancel
        </Button>
      </ModalFooter>
    </Modal>);
