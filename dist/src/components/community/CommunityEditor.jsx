...
export const CommunityEditor = () => {
    ...
    const hasChanged = () => {
        return (state.value?.title !== selectedCommunity.value?.title ||
    ...
    return (<Modal visible={communityEditorModalOpen} toggle={() => (communityEditorModalOpen.value = false)} onclose={reset}>
      <form onsubmit={saveCommunity}>
        ...
        <ModalBody>
          <div className="form-group">
            ...
            <input id="title" type="text" className="form-control" watch={state} bind:value={() => state.value?.title ?? ""} oninput={handleChange}/>
          </div>
          <div className="form-group">
            <label htmlFor="desc">Description</label>
            <textarea id="description" className="form-control" watch={state} bind:value={() => state.value?.description ?? ""} oninput={handleChange}/>
          </div>
          <div className="form-group flex-row">
            <label htmlFor="private">Private</label>
            <input id="private" type="checkbox" className="form-control" watch={state} bind:checked={() => state.value?.private ?? false} oninput={handleChange}/>
          </div>
        </ModalBody>
        <ModalFooter>
          <Button type="button" className="btn btn-secondary hover-animate" watch={loading} bind:disabled={() => loading.value} onclick={() => (communityEditorModalOpen.value = false)}>
            Cancel
          </Button>
          <Button watch={[loading, state]} bind:disabled={() => loading.value ||
            !communityValidation.isCommunityValid(state.value?.title ?? "", state.value?.description ?? "") ||
            !hasChanged()} className="btn btn-primary hover-animate">
            Save
